...
- [ ] **Phase 8: OBB UX Fix and Tech Debt Cleanup** - Close BENCH-01-cosmetic (OBB benchmark error message shows COCO layout instead of DOTA) and remove duplicate _check_backend_available in benchmark/_runner.py
- [ ] **Phase 9: Nyquist Validation** - Create VALIDATION.md files for all 7 existing phases to satisfy Nyquist compliance (all phases currently MISSING)

**Note:** CORR-04 (ONNX/CUDA server), CORR-05 (TensorRT/Jetson), CORR-06 (OpenVINO/Intel NUC) are formally deferred to v2.4 — hardware access required, tooling fully implemented.
...
### Phase 8: OBB UX Fix and Tech Debt Cleanup
**Goal**: Fix cosmetic OBB benchmark error message and remove duplicated internal function
**Depends on**: Phase 7
**Requirements**: BENCH-01 (cosmetic gap closure)
**Gap Closure:** Closes BENCH-01-cosmetic from v2.3-MILESTONE-AUDIT.md
**Success Criteria** (what must be TRUE):
  1. `yowo benchmark --model yolo11n-obb --data /nonexistent` shows DOTA layout hint, not COCO layout hint
  2. `benchmark/_runner.py` uses the public `check_backend_available` from Phase 3 instead of a local duplicate
**Plans**: 1 plan

Plans:
- [ ] 08-01-PLAN.md — Add OBB branch in benchmark_command data-path error (cli/_main.py:79) + remove duplicate _check_backend_available in benchmark/_runner.py

### Phase 9: Nyquist Validation
**Goal**: All 7 phases have VALIDATION.md files satisfying Nyquist compliance requirement (nyquist_validation: true)
**Depends on**: Phase 8
**Requirements**: None (process compliance)
**Success Criteria** (what must be TRUE):
  1. Each of the 7 phase directories contains a VALIDATION.md with test coverage, edge cases, and error paths verified
**Plans**: 1 plan

Plans:
- [ ] 09-01-PLAN.md — Generate VALIDATION.md for phases 01–07 via /gsd:validate-phase
...
| 8. OBB UX Fix and Tech Debt Cleanup | 0/1 | Pending    | - |
| 9. Nyquist Validation | 0/1 | Pending    | - |
