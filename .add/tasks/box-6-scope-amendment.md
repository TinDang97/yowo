...
verified:
  ...
  - { by: "cli", at: 2026-09-11, act: brief, authority: process, brief: "sha256:6295252f0493d428" }
  - { by: "process:run", at: 2026-09-11, act: run, authority: process, outcome: PASS, receipt: /tasks/box-6-scope-amendment.d/runs/1.md }
  - { by: "process:run", at: 2026-09-11, act: run, authority: process, outcome: PASS, receipt: /tasks/box-6-scope-amendment.d/runs/2.md }
  - { by: "Tin Dang", at: 2026-09-11, act: refreeze, authority: human, direction: "sha256:dd5a6d1c2c6fef43", binding: "sha256:d2b42533a0429356" }
  - { by: "process:run", at: 2026-09-11, act: run, authority: process, outcome: PASS, receipt: /tasks/box-6-scope-amendment.d/runs/3.md }
  - { by: "cli", at: 2026-09-11, act: brief, authority: process, brief: "sha256:f81d6c41aad6b87e" }
  - { by: "process:run", at: 2026-09-11, act: run, authority: process, outcome: PASS, receipt: /tasks/box-6-scope-amendment.d/runs/4.md }
  - { by: "Tin Dang", at: 2026-09-11, act: gate, authority: human, outcome: PASS, receipt: /tasks/box-6-scope-amendment.d/runs/4.md, brief: "sha256:f81d6c41aad6b87e", reason: "25 bound check items green on receipt 4, zero skipped, every Must and Reject bound. Suite 2428 passed / 11 skipped (the same chromadb and py38 skips that predate this node), ruff clean, ruff-format clean, pyright 0 errors. WHAT IS NOW TRUE. m1 box 6 enumerates userinfo, query and fragment, carries an AMENDED clause and a RESIDUAL RISK clause naming the path IN THE BOX BODY, and points at SECURITY.md. SECURITY.md gained a findable section stating what redaction covers, showing all four forms with transformations that are executed rather than transcribed, saying plainly that yowo does not redact the path and why, giving the safe rewrite, and telling the operator who CANNOT rewrite their URL that the stream id is then secret-bearing in logs, the RuntimeError, Frame.source_id and the cache keys. Both witnesses were re-derived whole -- assertion AND prose -- and now locate the box by its task citation instead of by a phrase from its own wording, which is what broke the moment the wording changed. REFUTED BY MUTATION, six executed: deleting the RESIDUAL RISK clause reddens 5; removing the SECURITY.md section reddens 6; adding path redaction to redact_url reddens 12; widening the box to also claim PATH reddens 1; dropping 'fragment' from the enumeration reddens 1; restoring the exact stale docstring #32 shipped reddens 1. That last is the point of the node: it proves test_the_box_claims_nothing_a_probe_refutes would have caught the defect that created this node, by EXECUTING the prose instead of reading it. THE GATE REFUSED ME THREE TIMES AND WAS RIGHT EVERY TIME. It refused PASS with M7 unbound, because test_no_redaction_behaviour_changed is parametrised and I cited it by its bare name -- which binds nothing against a reported id ending in [param]. That is method M12, filed two nodes ago, walked into by the node after it, and walked into again here by me; citations are now one line per parametrisation and E1 gained the citation it lacked, taking bound items 17 -> 25. It then refused because RULES/CHECKS had drifted after the freeze that approved them -- I had edited a frozen contract in place -- so the node was refrozen at human authority. It then refused R:UNBRIEFED, because the refrozen direction had never been compiled into a working prompt. Each refusal named its own fix and each fix was real bookkeeping, not a workaround. RED-FIRST, SPLIT HONESTLY. 11 of 23 items were red at the start and went green as the box, the document and the witnesses landed. The other 12 were green from the first run and could not have been otherwise: they are the M7, E1 and E4 pins asserting that redaction behaviour did NOT change, and a check whose subject is 'this is unchanged' cannot fail before the change it forbids. Recorded rather than contorted -- making them red would have meant breaking the code first. MY OWN CHECK WAS WRONG AND THE BUILD FOUND IT. test_the_box_claims_no_component_the_checks_do_not_establish scanned the box with a bare substring test and reported that it claimed 'host' -- matching inside 'hostage', in the box's own justification. Fixed with word boundaries. Same shape as the '1' that matched inside '10.0.0.5' one node ago: English embeds these words, so an 'in' scan over prose reports findings that are not there. A DEFECT IN ANOTHER NODE'S CHECKS, FOUND AND DELIBERATELY NOT FIXED. The new section turned two licensing-provenance checks red, and both are false positives in those checks rather than defects here: its email-intake regex matched 'hunter2@10.0.0.5' because an RTSP userinfo on a dotted host is indistinguishable from an email to that scanner, and its 'no LTS promise' check matched 'lts' inside 'results'. I changed MY examples -- an undotted host, and 'inference output' for 'inference results' -- rather than edit a closed node's checks to go green. Neither was weakened or touched. Both bugs are real and will fire again on innocent content; recorded as residual for the maintainer. SECURITY RESIDUE under the security-reviewer lens. No code changed: redact_url and safe_stream_id are pinned behaviourally against seven f9784fc outputs transcribed from that commit, and that pin is in the bound set. The disclosure question was weighed -- the section tells a reader a path-borne token reaches logs and cache keys -- and it is a defensive disclosure an operator needs in order to act, not a capability an attacker gains. No new trust boundary, no new input path, no concurrency surface. No security finding, so no HARD-STOP. SCOPE HELD. The box is AMENDED but NOT TICKED. Ticking is a separate 'add check' act at human authority, and the witnesses deliberately do not assert tick state (A15) so this node gates on what is true rather than on an ordering accident." }
...
- tests.unit.test_box_6_scope::test_no_redaction_behaviour_changed[https://cdn/hls/master.m3u8?st=abc&e=123#tok=Z] · covers: M7 · the redactor is pinned at f9784fc; this node moved a contract line and a document, not behaviour.
- tests.unit.test_box_6_scope::test_no_redaction_behaviour_changed[rtsp://cam:pw@[2001:db8::1/s] · covers: M7 · the redactor is pinned at f9784fc; this node moved a contract line and a document, not behaviour.
- tests.unit.test_box_6_scope::test_no_redaction_behaviour_changed[rtsp://camop:hunter2@10.0.0.5:554/s?token=SECRET&password=hunter2] · covers: M7 · the redactor is pinned at f9784fc; this node moved a contract line and a document, not behaviour.
- tests.unit.test_box_6_scope::test_no_redaction_behaviour_changed[rtsp://h/s?channel=1] · covers: M7 · the redactor is pinned at f9784fc; this node moved a contract line and a document, not behaviour.
- tests.unit.test_box_6_scope::test_no_redaction_behaviour_changed[rtsp://h/s?channel=2] · covers: M7 · the redactor is pinned at f9784fc; this node moved a contract line and a document, not behaviour.
- tests.unit.test_box_6_scope::test_no_redaction_behaviour_changed[rtsp://host:abc/path] · covers: M7 · the redactor is pinned at f9784fc; this node moved a contract line and a document, not behaviour.
- tests.unit.test_box_6_scope::test_no_redaction_behaviour_changed[rtsp://user:pw@h/live/S3CR3T/stream] · covers: M7 · the redactor is pinned at f9784fc; this node moved a contract line and a document, not behaviour.
- tests.unit.test_box_6_scope::test_the_path_form_is_still_the_exception · covers: E1 · the reason box 6 is qualified, re-derived against the running code rather than remembered.
