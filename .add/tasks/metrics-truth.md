...
- M4 A degradation increments a counter. OOM batch-halving, precision fallback and backend fallback each count, so an operator can tell a stream that degraded forty times from one that never did. · AMENDED 2026-09-15 by `precision-plumbing`: there are now TWO sites, not three. The precision-fallback site is RETIRED — `_try_precision_fallback` never performed a recovery at all (no backend has ever defined `set_precision`), so it counted a degradation for something that was never attempted, and `precision-plumbing` deleted the rung rather than keep a tier no check could execute. This Must was not wrong when it was gated; its third site turned out not to exist.
...
- A16 [absent] covers: S3 · the request does not say what a degradation that fails means — `_try_precision_fallback` catches its own exception; taking IT STILL COUNTS, because the engine still entered a degraded state -> if wrong, a failed recovery is invisible. · REFUTED 2026-09-15: the reading rested on a factual error. `_try_precision_fallback` did not attempt-and-fail; it attempted NOTHING — the `hasattr(self._backend, "set_precision")` guard has never once been true, because no backend in the tree defines that method. So nothing failed, and the count was phantom rather than generous. After `precision-plumbing` no degradation site can attempt something and fail, so this assumption HAS NO SUBJECT LEFT. Kept, struck through by this note rather than deleted, because a gated PASS cited it.
...
- E6 A degradation that fails (`_try_precision_fallback` swallowing its exception) — still counts. · RETIRED 2026-09-15: the edge names a case that never occurred (see A16). Its cited parametrize case `[precision]` was removed by `precision-plumbing` along with the rung it drove, and the remaining `[oom]` and `[backend]` cases still bind M4, A13 and A14.
...
- tests.unit.test_metrics_truth::test_each_degradation_site_increments_the_counter[precision] · covers: M4, A13, A14, A16, E6 · a recovery that fails still counts. · REMOVED 2026-09-15 by `precision-plumbing`, which deleted the rung this case drove. The receipt that gated this node ran it and it passed; that history stands. What it proved was that a phantom count increments, which is the defect `precision-plumbing`'s R:PHANTOM_DEGRADATION now forbids. Named here rather than silently dropped: a `done` node whose evidence changed underneath it should say so.
