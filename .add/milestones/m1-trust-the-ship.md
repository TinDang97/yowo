...
## EXIT
- [ ] CI builds the sdist+wheel twice under a pinned `SOURCE_DATE_EPOCH` and asserts identical digests; the digest is recorded in EVIDENCE so a local build can be diffed against it, and the sdist contains only files on an explicit include list   (← sdist-manifest, reproducible-sdist)
