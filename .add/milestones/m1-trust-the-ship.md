...
verified:
  ...
  - { by: "Tin Dang", at: 2026-09-11, act: check, authority: process, via: process, boxes: "EXIT:5" }
  - { by: "Tin Dang", at: 2026-09-11, act: check, authority: process, via: process, boxes: "EXIT:6" }
...
- [x] No downloaded weight is deserialized before its pinned SHA-256 is verified — on first download and on every cache hit — and `torch.load` runs with `weights_only=True`. The cache-migration policy for already-cached weights is decided by a human and recorded   (← weight-integrity)
- [x] No credential carried in a URL's USERINFO, QUERY or FRAGMENT reaches a log, an exception message, a result payload, or a cache key, asserted by checks that drive each of those three forms through both redaction boundaries and the routing-callback key, with the userinfo and query forms additionally bound at all four sinks. AMENDED 2026-09-11 from "No credential reaches a log, an exception message, a result payload, or a cache key" by human decision: that wording is unsatisfiable by any change to this repository, and an unsatisfiable box either holds the milestone hostage or gets ticked while false. RESIDUAL RISK, named rather than hidden: a secret carried in the URL path is NOT redacted and cannot be — `rtsp://h/live/S3CR3T-signed/stream` -> unchanged — because the path IS the camera's identity; emptying it would collapse every camera on a host onto one stream id, and `DetectionRouter.register` overwrites a duplicate key silently, so one camera's detections would be delivered to another's callback. That is the cross-delivery bug `credential-sinks-are-bound` fixed at the cause, and re-introducing it to satisfy a sentence is a strictly worse trade. Path-borne secrets are an OPERATOR CONSTRAINT, documented in SECURITY.md   (← rtsp-credential-redaction, credential-sinks-are-bound, query-string-credentials, box-6-scope-amendment)
